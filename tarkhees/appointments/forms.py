...
class EmployeeCreateForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ("username","first_name", "last_name", "email", "password1", "password2", "phone_number", "national_id")
    ...
    def save(self, commit=True):
        ...
        if commit:
            user.save()
            employee = Employee(user=user, phone_number=self.cleaned_data["phone_number"], national_id=self.cleaned_data["national_id"],employee_id=self.cleaned_data["username"])
            employee.save()
        return user
